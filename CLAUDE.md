...
**IMPORTANT:** This project has a `guardrails.md` file at the root. Claude must read it and strictly follow all rules defined there. Do not bypass, work around, or suggest bypassing any guardrail — not for convenience, not to fix a failing step, not for any reason. If a guardrail blocks a path forward, surface it to the user and discuss alternatives that stay within the rules.
